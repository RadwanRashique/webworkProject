...
function Dashboard() {
  ...
  const getDeveloperData = async () => {
    toast.success("developer Details")
    dispatch(showLoading())
    adminRequest({
      url: apiEndPoints.developerdetails,
      method: 'get',

    }).then((response) => {
      dispatch(hideLoading())
      if (response.data.success) {
        setDeveloperData(response.data.developerData)
      ...
      dispatch(hideLoading())
      console.log(error, "at DeveloperDetails")
  ...
  const blockdeveloper = async (id) => {
    const isBlocked = developerData.find((developer) => developer._id === id)?.isBlocked;
    ...
    if (result.isConfirmed) {
      ...
      adminRequest({
        url: apiEndPoints.blockdeveloper,
        method: 'post',
        data: { id: id },
  ...
  const handleSearch = (val) => {
    setSearchInput(val)
    if (val === '') {
      setFilteredData(developerData?.slice(currentPage * itemsPerPage, (currentPage + 1) * itemsPerPage));

    }
    else {
      const res = developerData.filter((item) =>
  ...
  useEffect(() => {
    ...
    setFilteredData(developerData.slice(startIndex, endIndex));
  }, [currentPage, developerData]);
      ...
      <div className="flex flex-col md:flex-row p-4 md:p-8">
        ...
        <div className="flex-grow md:w-1/2">
          <div className="flex flex-col md:flex-row items-center justify-between mb-4 md:mb-8">
            <h1 className="text-2xl md:text-3xl font-semibold mb-2 md:mb-0">Developer Details</h1>
            <input
              type="text"
              ...
              {filteredData.length > 0 && (
                <thead>
                  <tr>
                    ...
                    <th className="px-2 md:px-4 py-2">Developer Id</th>

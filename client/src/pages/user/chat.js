...
const chat = () => {
  ...
  const developerId = location.state
  ...
  const fetchChatMessages = async (developerId) => {
  ...
  useEffect(() => {
    fetchChatMessages(developerId);

  }, [msg])

  const sendChatMessage = async (room_id, developerId) => {
      ...
      <div className="flex-1 p-2 sm:p-6 justify-between flex flex-col">
        ...
        <div className="flex sm:items-center justify-between py-3 border-b-2 border-gray-200 w-full">
          <div className="relative flex items-center space-x-4 w-full">
            <div className="relative">
              ...
              <img
                src={chatPartner?.developerImage}
                alt={chatPartner?.developerName}
                className="w-10 sm:w-16 h-10 sm:h-16 rounded-full"
              />
            </div>
            <div className="flex flex-col leading-tight">
              <div className="text-2xl mt-1 flex items-center">
                <span className="text-gray-700 mr-3">{chatPartner?.developerName}</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="flex-grow w-full overflow-y-auto h-screen" ref={chatContainerRef}>
          ...
          {chatHistory?.map((message) => {
            const isUserMessage = message.senderId === message.userId;
        ...
        <div className="border-t-2 w-full border-gray-200 px-4 pt-4 mb-2 sm:mb-0">
          <div className="relative flex">
            ...
            <div className="absolute right-0 items-center inset-y-0 flex">
              <button
                type="button"
                onClick={() => { sendChatMessage(chatPartner?._id, chatPartner?.developerId) }}
